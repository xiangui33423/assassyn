...
# Cache for library paths to avoid repeated file I/O
_CWRAPPER_LIB_PATH_CACHE = None
_RAMULATOR2_LIB_PATH_CACHE = None

def cwrapper_lib_path():
    """Get the path to the C wrapper shared library.

    Reads the path from the .cwrapper-lib-path file generated by CMake.
    ...
    if _CWRAPPER_LIB_PATH_CACHE is None:
        ...
        with open(path_file, 'r', encoding='utf-8') as f:
            _CWRAPPER_LIB_PATH_CACHE = f.read().strip()
    return _CWRAPPER_LIB_PATH_CACHE

def ramulator2_lib_path():
    """Get the path to the Ramulator2 shared library.

    Reads the path from the .ramulator2-lib-path file generated by CMake.
    Uses caching to avoid repeated file I/O.
    ...
    if _RAMULATOR2_LIB_PATH_CACHE is None:
        ...
        with open(path_file, 'r', encoding='utf-8') as f:
            _RAMULATOR2_LIB_PATH_CACHE = f.read().strip()
    return _RAMULATOR2_LIB_PATH_CACHE
...
# Load libraries using the new utility methods
wrapper_lib_path = cwrapper_lib_path()
ramulator_lib_path = ramulator2_lib_path()
