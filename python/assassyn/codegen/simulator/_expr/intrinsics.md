...
Generates code to read a value from an `ExternalIntrinsic` output. The helper computes the per-instance handle name (`external_<uid>`) and returns a clone of the requested field.
